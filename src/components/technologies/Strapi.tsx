import * as React from "react";
export default function Strapi(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
    viewBox="0 0 600 600"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M0 208C0 109.948 0 60.922 30.46 30.46 60.923 0 109.949 0 208 0h184c98.052 0 147.078 0 177.539 30.46C600 60.923 600 109.949 600 208v184c0 98.052 0 147.078-30.461 177.539S490.052 600 392 600H208c-98.052 0-147.078 0-177.54-30.461C0 539.078 0 490.052 0 392z"
      fill="#4945ff"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M414 182H212v103h103v103h103V186a4 4 0 0 0-4-4"
      fill="#fff"
    />
    <path fill="#fff" d="M311 285h4v4h-4z" />
    <path
      d="M212 285h99a4 4 0 0 1 4 4v99h-99a4 4 0 0 1-4-4zm103 103h103l-99.586 99.586c-1.26 1.26-3.414.367-3.414-1.414zM212 285h-98.172c-1.782 0-2.674-2.154-1.414-3.414L212 182z"
      fill="#9593ff"
    />
  </svg>
);
}
